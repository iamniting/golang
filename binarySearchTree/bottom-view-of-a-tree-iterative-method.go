...
func VerticalOrderIterative(root * Node) {
    ...
    type vertical struct { node *Node; hd int }
    ...
    var hds []int
    for hd := range m { hds = append(hds, hd) }
    sort.Ints(hds)

    // print bottom view via vertical order
    for _, hd := range hds { fmt.Println(m[hd][len(m[hd]) - 1]) }
}
...
func main() {
    ...
    fmt.Println("\n\nBottom View of a Tree Iteratively")
